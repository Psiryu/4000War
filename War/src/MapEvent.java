 ...
 * MapEvent Class
 *
 * Description: This class mediates between the user interface and the back end
 * mechanics, namely the movement and combat mechanics. Several lists are
 * maintained by this class, each of which records a particular aspect of the
 * behavior of units as they are manipulated through the front end. Subsequently
 * the methods in this class serve to manipulate the back end. These methods are
 * largely private as they are intended for internal use only. Methods such as
 * addMovement, mergeUnit, and divideUnit are used by the interface to
 * manipulate units while completeCombat and successfulFlee are used by the
 * Battle class to facilitate movement and unit deletion after combat
 * calculations. Internal methods include processEvents, removeInstance,
 ...
 *
 * Usage: MapEvent is used extensively in AI, Battle, and Map. These classes
 * call to MapEvent when an event is processed regarding any unit in play. The
 * MapEvent object is initialized in Global and is called to in Game.endTurn().
 *
 * Maintenance notes: As MapEvent serves as a background for all game mechanics,
 * there is no explicit need to modify this class. Maintenance to any method
 * should be performed after consulting method specific documentation.
 */
public class MapEvent {
	...
	private static ArrayList<CombatUnit> removeQueue = new ArrayList<CombatUnit>(); // units that have not participated in a battle allowed to move to their intended location
	private static ArrayList<Integer> postCombatUnit = new ArrayList<Integer>(); // list of units which have successfully fled a battle instance and require additional processing
	private static ArrayList<Road> postCombatRoad = new ArrayList<Road>(); // information regarding a fleeing unit's road upon which they are traveling
	private static ArrayList<Integer> postCombatNode = new ArrayList<Integer>(); // information regarding a fleeing unit's end location
	...
	/*
	 Method: addMovement
	 Input Parameters: int unitNum -> ID of the unit being moved
	...
	/*
	 ...
	 Input Parameters: CombatUnit unit -> the unit which is to be removed from the movement lists
	 Output Parameters: none
	...
	private static void removeInstances() {
		...
		// reset all lists pretaining to the turn which has passed
		...
		// add any fleeing units to the refreshed MapEvent instance
		for (int i = 0; i < postCombatUnit.size(); i++) {
			addMovement(postCombatUnit.get(i), postCombatRoad.get(i), postCombatNode.get(i));
		}

		// remove all fleeing units from the lists storing the flee
	...
	/*
	 ...
	 Input Parameters: ArrayList<CombatUnit> redUnits -> list of red units in combat
	...
	/*
	 ...
	 Input Parameters: ArrayList<CombatUnit> units -> the units that participated in the flee
	...
	/*
	 Method: mergeUnits
	 Input Parameters: int oneNum -> the ID of the first unit
	...
	/*
	 Method: divideUnit
	 Input Parameters: int unitNum -> the ID of unit to be divided
	 Output Parameters: none
